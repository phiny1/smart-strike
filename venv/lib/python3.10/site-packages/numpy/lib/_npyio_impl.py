    ...
    with contextlib.ExitStack() as stack:
        ...
        if magic.startswith((_ZIP_PREFIX, _ZIP_SUFFIX)):
            ...
            if mmap_mode:
                ...
                return format.open_memmap(
                    file, mode=mmap_mode, max_header_size=max_header_size
                )
            else:
                return format.read_array(
                    fid,
        ...
        if iscomplex_X:
            ...
            for row in X:
                try:
                    ...
                    raise TypeError(
                        "Mismatch between array dtype ('%s') and "
                        "format specifier ('%s')" % (str(X.dtype), format)
                    ) from e
                fh.write(v)
    ...
    * Custom converters may receive unexpected values due to dtype
    ...
    with fid_ctx:
        ...
        if dtype is None:
            ...
            if len(dtype_flat) > 1:
                ...
                converters = [
                    StringConverter(dt, locked=True, missing_values=miss, default=fill)
    ...
    if loose:
        rows = list(
            zip(
                *[
                    [conv._loose_call(_r) for _r in map(itemgetter(i), rows)]
        ...
        rows = list(
            zip(
                *[
                    [conv._strict_call(_r) for _r in map(itemgetter(i), rows)]
                    for (i, conv) in enumerate(converters)
                ]
            )
        )
