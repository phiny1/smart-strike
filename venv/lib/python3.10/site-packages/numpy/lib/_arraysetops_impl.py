...
def in1d(ar1, ar2, assume_unique=False, invert=False, *, kind=None):
    ...
    warnings.warn(
        "`in1d` is deprecated. Use `np.isin` instead.", DeprecationWarning, stacklevel=2
    )
    ...
    if use_table_method:
        ...
        if range_safe_from_overflow and (below_memory_constraint or kind == "table"):
            ...
            outgoing_array[basic_mask] = isin_helper_ar[
                np.subtract(
                    in_range_ar1, ar2_min, dtype=dtype, out=out, casting="unsafe"
                )
            ]
...
def _isin_dispatcher(
    element, test_elements, assume_unique=None, invert=None, *, kind=None
):
