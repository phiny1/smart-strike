...
def _read_array_header(fp, version, max_header_size=_MAX_HEADER_SIZE):
    ...
    if not isinstance(d["shape"], tuple) or not all(
        isinstance(x, int) for x in d["shape"]
    ):
...
def read_array(
    fp, allow_pickle=False, pickle_kwargs=None, *, max_header_size=_MAX_HEADER_SIZE
):
    """
...
def open_memmap(
    ...
    *,
    ...
    marray = numpy.memmap(
        filename, dtype=dtype, shape=shape, order=order, mode=mode, offset=offset
    )
