            ...
            if "*" in var["kindselector"]:
                ...
                try:
                    ...
                    try:
                        ...
                        errmess(
                            'getctype: "%s(kind=%s)" is mapped to C "%s" (to override define dict(%s = dict(%s="<C typespec>")) in %s/.f2py_f2cmap file).\n'
                            % (
                                typespec,
                                var["kindselector"]["kind"],
                                ctype,
                                typespec,
                                var["kindselector"]["kind"],
                                os.getcwd(),
...
def getpydocsign(a, var):
    ...
    if isscalar(var):
        ...
        dim = var["dimension"]
        rank = repr(len(dim))
        sig = "%s : %s rank-%s array('%s') with bounds (%s)%s" % (
            a,
            opt,
            rank,
            c2pycode_map[ctype],
...
def getarrdocsign(a, var):
    ...
    if isstring(var) and (not isarray(var)):
        ...
        dim = var["dimension"]
        rank = repr(len(dim))
        sig = "%s : rank-%s array('%s') with bounds (%s)" % (
            a,
            rank,
            c2pycode_map[ctype],
            ",".join(dim),
...
def sign2map(a, var):
    ...
    if debugcapi(var):
        il = [
            ...
            iscomplex,
            "complex scalar",
            l_and(isscalar, l_not(iscomplex)),
            "scalar",
            isstring,
            "string",
            isarray,
            ...
            iscomplexfunction,
            "complex function",
            l_and(isfunction, l_not(iscomplexfunction)),
            "function",
            isexternal,
            "callback",
            isintent_callback,
...
def routsign2map(rout):
    ...
    if "use" in rout:
        ...
        errmess(
            'routsign2map: Confused: function %s has externals %s but no "use" statement.\n'
    ...
    if isfunction(rout):
        ...
        if isstringfunction(rout):
            ...
            if ret["rlength"] == "-1":
                errmess(
                    "routsign2map: expected explicit specification of the length of the string returned by the fortran function %s; taking 10.\n"
                    % (repr(rout["name"]))
                )
                ret["rlength"] = "10"
    if hasnote(rout):
