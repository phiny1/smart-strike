...
def run_main(comline_list):
    ...
    if args.module_name:
        if "-h" in comline_list:
            modname = args.module_name  # Directly use from args when -h is present
        else:
...
def f2py_parser():
    ...
    parser.add_argument(
        "--include-paths", dest="include_paths", action=CombineIncludePaths
    )
    parser.add_argument(
        "--include_paths", dest="include_paths", action=CombineIncludePaths
    )
    parser.add_argument(
...
def preparse_sysargv():
    ...
    if MESON_ONLY_VER and backend_key == "distutils":
        outmess(
            "Cannot use distutils backend with Python>=3.12,"
