...
def isvariable(var):
    ...
    if (
        len(var) == 1
        and "attrspec" in var
        and var["attrspec"][0] in ("public", "private")
    ):
        is_var = False
    else:
        is_var = True
...
_f2py_module_name_match = re.compile(
    r"\s*python\s*module\s*(?P<name>[\w_]+)", re.I
).match
_f2py_user_module_name_match = re.compile(
    r"\s*python\s*module\s*(?P<name>[\w_]*?" r"__user__[\w_]*)", re.I
).match
