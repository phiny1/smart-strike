...
def buildhooks(pymod):
    ...
    for m in findf90modules(pymod):
        ...
        for n in m["vars"].keys():
            ...
            if (n not in notvars and isvariable(var)) and (
                not l_or(isintent_hide, isprivate)(var)
            ):
                onlyvars.append(n)
                mfargs.append(n)
        ...
        if m["name"] in usenames and containscommon(m):
            outmess(
                f"\t\t\tSkipping {m['name']} since it is in 'use' and contains a common block...\n"
            )
            continue
        if onlyvars:
            outmess("\t\t  Variables: %s\n" % (" ".join(onlyvars)))
        ...
        for n in onlyvars:
            ...
            cadd(
                '\t{"%s",%s,{{%s}},%s, %s},'
                % (undo_rmbadname1(n), dm["rank"], dms, at, capi_maps.get_elsize(var))
            )
        ...
        ret["initf90modhooks"] = [
            '\tPyDict_SetItemString(d, "%s", PyFortranObject_New(f2py_%s_def,f2py_init_%s));'
