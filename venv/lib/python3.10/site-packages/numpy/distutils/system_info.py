...
if sys.platform == "win32":
    ...
    default_lib_dirs = libpaths(
        ["/usr/local/lib", "/opt/lib", "/usr/lib", "/opt/local/lib", "/sw/lib"],
        platform_bits,
    )
    default_runtime_dirs = []
    default_include_dirs = [
    ...
    default_x11_lib_dirs = libpaths(
        ["/usr/X11R6/lib", "/usr/X11/lib", "/usr/lib"], platform_bits
    )
    ...
    if os.path.exists("/usr/lib/X11"):
        ...
        if globbed_x11_dir:
            ...
            default_x11_include_dirs.extend(
                ["/usr/lib/X11/include", "/usr/include/X11"]
            )

    with open(os.devnull, "w") as tmp:
        try:
            p = subprocess.Popen(
                ["gcc", "-print-multiarch"], stdout=subprocess.PIPE, stderr=tmp
            )
...
def _parse_env_order(base_order, env):
    ...
    if neg:
        if sum_neg > 1:
            raise ValueError(
                f"Environment variable '{env}' may only contain a single (prefixed) negation: {order_str}"
            )
        # remove prefix
        order_str = order_str[1:]
    elif sum_neg > 0:
        raise ValueError(
            f"Environment variable '{env}' may not mix negated an non-negated items: {order_str}"
        )
...
class system_info:
    ...
    def __init__(
        ...
        self.local_prefixes = []
        defaults = {
            "library_dirs": os.pathsep.join(default_lib_dirs),
    ...
    def get_option_single(self, *options):
        ...
        *options : list of str
        ...
        raise AliasedOptionError(
            AliasedOptionError.__doc__.format(
        ...
        if env_var and env_var in os.environ:
            ...
            if os.path.isfile(d):
                ...
                if len(l) == 1:
                    ...
                    if b[:3] == "lib":
                        log.info(
                            "Replacing _lib_names[0]==%r with %r"
...
class fftw_info(system_info):
    ...
    def calc_ver_info(self, ver_param):
        ...
        if info is not None:
            flag = 0
            for d in incl_dirs:
                if len(self.combine_paths(d, ver_param["includes"])) == len(
                    ver_param["includes"]
                ):
    ...
    def calc_info(self):
        ...
        for d in incl_dirs:
            if len(self.combine_paths(d, ["fftc8.h", "fftfreq.h"])) == 2:
                dict_append(
                    info, include_dirs=[d], define_macros=[("SCIPY_DJBFFT_H", None)]
                )
                self.set_info(**info)
                return
        return
...
class ssl2_info(system_info):
    ...
    def calc_info(self):
        ...
        dict_append(
            info,
            define_macros=[("HAVE_CBLAS", None), ("HAVE_SSL2", 1)],
            include_dirs=incl_dirs,
        )
        self.set_info(**info)
...
class atlas_info(system_info):
    ...
    def get_paths(self, section, key):
        ...
        for d in pre_dirs:
            dirs.extend(
                self.combine_paths(d, ["atlas*", "ATLAS*", "sse", "3dnow", "sse2"])
                + [d]
            )
        return [d for d in dirs if os.path.isdir(d)]
...
class lapack_src_info(system_info):
    ...
    def calc_info(self):
        ...
        sources = (
            ["s%s.f" % f for f in (sclaux + slasrc).split()]
...
def get_atlas_version(**config):
    ...
    try:
        ...
        if atlas_version:
            dict_append(
                info, define_macros=[("ATLAS_INFO", _c_string_literal(atlas_version))]
            )
        else:
...
class lapack_opt_info(system_info):
    ...
    def calc_info(self):
        lapack_order, unknown_order = _parse_env_order(
            self.lapack_order, self.order_env_var_name
        )
        if len(unknown_order) > 0:
            raise ValueError(
                "lapack_opt_info user defined "
...
class blas_opt_info(system_info):
    ...
    def calc_info(self):
        blas_order, unknown_order = _parse_env_order(
            self.blas_order, self.order_env_var_name
        )
        if len(unknown_order) > 0:
            raise ValueError(
                "blas_opt_info user defined BLAS order has unacceptable values: {}".format(
                    unknown_order
                )
            )
    ...
    def get_cblas_libs(self, info):
        ...
        try:
            ...
            try:
                ...
                obj = c.compile(
                    [src], output_dir=tmpdir, include_dirs=self.get_include_dirs()
                )
...
class openblas_info(blas_info):
    ...
    def check_symbols(self, info):
        ...
        prototypes = "\n".join(
            "void %s%s%s();" % (self.symbol_prefix, symbol_name, self.symbol_suffix)
            for symbol_name in self._require_symbols
        )
        calls = "\n".join(
            "%s%s%s();" % (self.symbol_prefix, symbol_name, self.symbol_suffix)
            for symbol_name in self._require_symbols
        )
        s = (
            textwrap.dedent(
...
class accelerate_info(system_info):
    ...
    def calc_info(self):
        ...
        if sys.platform == "darwin" and not os.getenv("_PYTHON_HOST_PLATFORM", None):
            ...
            if (
                get_platform()[-4:] == "i386"
                or "intel" in get_platform()
                or "x86_64" in get_platform()
                or "i386" in platform.platform()
            ):
                intel = 1
            else:
                intel = 0
            if (
                ...
                os.path.exists("/System/Library/Frameworks" "/vecLib.framework/")
                and "veclib" in libraries
            ):
                if intel:
                    args.extend(["-msse3"])
                args.extend(["-I/System/Library/Frameworks/vecLib.framework/Headers"])
...
class blas_src_info(system_info):
    ...
    def calc_info(self):
        ...
        """
        sources = [
            os.path.join(src_dir, f + ".f") for f in (blas1 + blas2 + blas3).split()
        ]
...
class _numpy_info(system_info):
    ...
    def __init__(self):
        ...
        system_info.__init__(
            self, default_lib_dirs=[], default_include_dirs=include_dirs
        )
    ...
    def calc_info(self):
        ...
        for v in ["__version__", "version"]:
            ...
            if vrs is None:
                continue
            macros = [
                (self.modulename.upper() + "_VERSION", _c_string_literal(vrs)),
...
class numerix_info(system_info):
    ...
    def calc_info(self):
        ...
        if which[0] not in ["numeric", "numarray", "numpy"]:
            raise ValueError(
                "numerix selector must be either 'Numeric' "
...
class f2py_info(system_info):
    def calc_info(self):
        ...
        self.set_info(
            sources=[os.path.join(f2py_dir, "fortranobject.c")], include_dirs=[f2py_dir]
        )
        return
    ...
    def calc_info(self):
        ...
        info = {
            "libraries": [
                (
                    "boost_python_src",
                    {"include_dirs": [src_dir] + py_incl_dirs, "sources": bpl_srcs},
                )
            ],
...
class agg2_info(system_info):
    ...
    def calc_info(self):
        ...
        if sys.platform == "win32":
            agg2_srcs = glob(
                os.path.join(src_dir, "src", "platform", "win32", "agg_win32_bmp.cpp")
            )
        else:
            agg2_srcs = glob(os.path.join(src_dir, "src", "*.cpp"))
            agg2_srcs += [
                os.path.join(
                    src_dir, "src", "platform", "X11", "agg_platform_support.cpp"
                )
            ]
...
class _pkg_config_info(system_info):
    ...
    def calc_info(self):
        ...
        if not config_exe:
            log.warn(
                "File not found: %s. Cannot determine %s info."
        ...
        if version:
            macros.append(
                (
                    self.__class__.__name__.split(".")[-1].upper(),
            ...
            if self.version_macro_name:
                macros.append(
                    (
                        self.version_macro_name + "_%s" % (version.replace(".", "_")),
                        None,
                    )
                )
        if self.release_macro_name:
            ...
            if release:
                macros.append(
                    (
                        self.release_macro_name + "_%s" % (release.replace(".", "_")),
                        None,
                    )
                )
