...
class TestBincount(_DeprecationTestCase):
    ...
    # 2024-07-29, 2.1.0
    @pytest.mark.parametrize("badlist", [[0.5, 1.2, 1.5], ["0", "1", "1"]])
...
class TestRemovedGlobals:
    ...
    # (or faster).
    @pytest.mark.parametrize("name", ["object", "float", "complex", "str", "int"])
...
class TestDeprecatedSaveFixImports(_DeprecationTestCase):
    ...
    def test_deprecated(self):
        with temppath(suffix=".npy") as path:
            ...
            self.assert_deprecated(
                np.save, args=sample_args, kwargs={"fix_imports": True}
            )
            self.assert_deprecated(
                np.save, args=sample_args, kwargs={"fix_imports": False}
            )
            for allow_pickle in [True, False]:
                self.assert_not_deprecated(
                    np.save, args=sample_args, kwargs={"allow_pickle": allow_pickle}
                )
                self.assert_deprecated(
                    np.save,
                    args=sample_args,
                    kwargs={"allow_pickle": allow_pickle, "fix_imports": True},
                )
                self.assert_deprecated(
                    np.save,
                    args=sample_args,
                    kwargs={"allow_pickle": allow_pickle, "fix_imports": False},
                )
