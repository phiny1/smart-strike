...
class TestBuiltin:
    ...
    def test_bad_param(self):
        ...
        assert_raises(
            ValueError,
            np.dtype,
            {"names": ["f0", "f1"], "formats": ["i1", "f4"], "offsets": [0, 2]},
            align=True,
        )
...
class TestRecord:
    ...
    def test_aligned_size(self):
        ...
        assert_equal(dt.itemsize, 8)
        dt = np.dtype(
            {"names": ["f0", "f1"], "formats": ["i4", "u1"], "offsets": [0, 4]},
            align=True,
        )
        assert_equal(dt.itemsize, 8)
        ...
        dt2 = np.dtype(
            {
                "names": ["f0", "f1", "f2"],
                "formats": ["i4", [("f1", "i1"), ("f2", "i4"), ("f3", "i1")], "i1"],
                "offsets": [0, 4, 16],
            },
            align=True,
        )
    ...
    def test_union_struct(self):
        ...
        assert_raises(
            TypeError,
            np.dtype,
            {"names": ["f0", "f1"], "formats": ["O", "i1"], "offsets": [0, 2]},
        )
        assert_raises(
            TypeError,
            np.dtype,
            {"names": ["f0", "f1"], "formats": ["i4", "O"], "offsets": [0, 3]},
        )
        assert_raises(
            TypeError,
            np.dtype,
            {"names": ["f0", "f1"], "formats": [[("a", "O")], "i1"], "offsets": [0, 2]},
        )
        assert_raises(
            TypeError,
            np.dtype,
            {"names": ["f0", "f1"], "formats": ["i4", [("a", "O")]], "offsets": [0, 3]},
        )
    ...
    def test_parenthesized_single_number(self):
        ...
        with pytest.warns(DeprecationWarning, match="parenthesized single number"):
            np.dtype("(2)f4,")
    ...
    def test_multifield_index(self, align_flag):
        ...
        assert_equal(
            dt_sub,
            np.dtype({"names": [], "formats": [], "offsets": [], "itemsize": 96}),
        )
        assert_equal(dt_sub.isalignedstruct, align_flag)
...
class TestStructuredObjectRefcounting:
    ...
    def test_structured_object_create_delete(
        self, dt, pat, count, singleton, creation_func, creation_obj
    ):
    ...
    def test_structured_object_indexing(
        self, shape, index, items_changed, dt, pat, count, singleton
    ):
...
class TestStructuredDtypeSparseFields:
    ...
    dtype = np.dtype(
        [
            (
                "a",
                {"names": ["aa", "ab"], "formats": ["f", "f"], "offsets": [0, 4]},
                (2, 3),
            )
        ]
    )
...
class TestString:
    def test_complex_dtype_str(self):
        dt = np.dtype(
            [
                (
                    "top",
                    [("tiles", (">f4", (64, 64)), (1,)), ("rtile", ">f4", (64, 36))],
                    (3,),
                ),
                (
                    "bottom",
                    [("bleft", (">f4", (8, 64)), (1,)), ("bright", ">f4", (8, 36))],
                ),
            ]
        )
        assert_equal(
        ...
        dt = np.dtype(
            {
                ...
                "formats": ["<u4", "u1", "u1", "u1"],
                "offsets": [0, 0, 1, 2],
                "titles": ["Color", "Red pixel", "Green pixel", "Blue pixel"],
            }
        )
        assert_equal(
            str(dt),
    ...
    def test_repr_structured_datetime(self):
        dt = np.dtype([("a", "<M8[D]"), ("b", "<m8[us]")])
        assert_equal(repr(dt), "dtype([('a', '<M8[D]'), ('b', '<m8[us]')])")
...
class TestPickling:
    ...
    @pytest.mark.parametrize("t", [int, float, complex, np.int32, str, object, bool])
    ...
    @pytest.mark.parametrize(
        "dt", [np.dtype(t) for t in np.typecodes["All"]] + [np.dtype(rational)]
    )
...
class TestPromotion:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (np.longdouble(np.nextafter(1.7e308, 0.0)), np.clongdouble),
            ...
            # Base of the double value to sidestep any rounding issues:
            (np.clongdouble(np.nextafter(1.7e308, 0.0) * 1j), np.clongdouble),
            # Additionally use "nextafter" so the cast can't round down:
