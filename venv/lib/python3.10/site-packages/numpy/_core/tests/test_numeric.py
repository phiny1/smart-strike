...
class TestNonarrayArgs:
    ...
    def test_reshape_shape_arg(self):
        ...
        with pytest.raises(
            TypeError,
            match=r"reshape\(\) missing 1 required positional " "argument: 'shape'",
        ):
            np.reshape(arr)
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                2**31 - 1, -1, marks=pytest.mark.skip(reason="Out of range of int32")
            ),
    ...
    def test_var_with_mean_keyword_keepdims_false(self):
        ...
        var = np.var(A, axis=1, keepdims=False, mean=mean)
    ...
    def test_std_with_mean_keyword_where_nontrivial(self):
        ...
        mean = np.mean(A, axis=1, keepdims=True, where=where)

        std = np.std(A, axis=1, keepdims=False, mean=mean, where=where)
    ...
    def test_var_with_mean_keyword_where_nontrivial(self):
        ...
        var = np.var(A, axis=1, keepdims=False, mean=mean, where=where)
    ...
    def test_std_with_mean_keyword_multiple_axis(self):
        ...
        mean = np.mean(A, out=None, axis=axis, keepdims=True)

        std = np.std(A, out=None, axis=axis, keepdims=False, mean=mean)
    ...
    def test_std_with_mean_keyword_keepdims_true_masked(self):
        ...
        B = ma.array(
            [[100.0, 3.0, 104.0, 5.0], [101.0, 2.0, 103.0, 4.0]],
            mask=[[True, False, True, False], [True, False, True, False]],
        )

        mean_out = ma.array([[0.0, 0.0, 0.0, 0.0]], mask=[[False, False, False, False]])
        std_out = ma.array([[0.0, 0.0, 0.0, 0.0]], mask=[[False, False, False, False]])

        axis = 0

        mean = np.mean(A, out=mean_out, axis=axis, keepdims=True)

        std = np.std(A, out=std_out, axis=axis, keepdims=True, mean=mean)
    ...
    def test_var_with_mean_keyword_keepdims_true_masked(self):
        ...
        mean_out = ma.array([[0.0, 0.0, 0.0, 0.0]], mask=[[False, False, False, False]])
        var_out = ma.array([[0.0, 0.0, 0.0, 0.0]], mask=[[False, False, False, False]])

        axis = 0

        mean = np.mean(A, out=mean_out, axis=axis, keepdims=True)

        var = np.var(A, out=var_out, axis=axis, keepdims=True, mean=mean)
...
class TestFloatExceptions:
    ...
    def test_floating_exceptions(self, typecode):
        if "bsd" in sys.platform and typecode in "gG":
            pytest.skip(
                reason="Fallback impl for (c)longdouble may not raise "
...
class TestTypes:
    def check_promotion_cases(self, promote_func):
        ...
        assert_equal(promote_func(fld, np.array([f32])), np.dtype(np.longdouble))
        assert_equal(promote_func(np.array([f64]), fld), np.dtype(np.longdouble))
        assert_equal(promote_func(fld, np.array([c64])), np.dtype(np.clongdouble))
        ...
        assert_equal(promote_func(np.array([f32]), c128), np.dtype(np.complex128))
    ...
    @pytest.mark.parametrize("dtype", list("?bhilqBHILQefdgFDG") + [rational])
...
class TestClip:
    ...
    def test_clip_min_max_args(self):
        ...
        assert_array_equal(np.clip(arr, min=None, max=2), np.clip(arr, None, 2))

        with assert_raises_regex(
            TypeError, "missing 1 required positional " "argument: 'a_max'"
        ):
            np.clip(arr, 2)
        with assert_raises_regex(
            TypeError, "missing 1 required positional " "argument: 'a_min'"
        ):
            np.clip(arr, a_max=2)
        msg = (
...
class TestIsclose:
    ...
    def test_nep50_isclose(self):
        below_one = float(1.0 - np.finfo("f8").eps)
    ...
    def test_ip_all_isclose(self):
        ...
        assert np.allclose(
            [1, 2, float("nan")], [1, 2, float("nan")], atol=[1, 1, 1], equal_nan=True
        )
...
class TestStdVar:
    ...
    def test_correction(self):
        assert_almost_equal(np.var(self.A, correction=1), np.var(self.A, ddof=1))
        assert_almost_equal(np.std(self.A, correction=1), np.std(self.A, ddof=1))
...
class TestMoveaxis:
    ...
    def test_errors(self):
        ...
        assert_raises_regex(AxisError, "source.*out of bounds", np.moveaxis, x, 3, 0)
        assert_raises_regex(AxisError, "source.*out of bounds", np.moveaxis, x, -4, 0)
        assert_raises_regex(
            AxisError, "destination.*out of bounds", np.moveaxis, x, 0, 5
        )
        assert_raises_regex(
...
class TestCross:
    @pytest.mark.filterwarnings("ignore:.*2-dimensional vectors.*:DeprecationWarning")
    def test_2x2(self):
        u = [1, 2]
        v = [3, 4]
        z = -2
...
class TestAsType:
    def test_astype(self):
        data = [[1, 2], [3, 4]]
        actual = np.astype(np.array(data, dtype=np.int64), np.uint32)
        expected = np.array(data, dtype=np.uint32)

        assert_array_equal(actual, expected)
        assert_equal(actual.dtype, expected.dtype)

        assert np.shares_memory(actual, np.astype(actual, actual.dtype, copy=False))
