    ...
    while len(modules_queue) > 0:
        ...
        for member_name in dir(module):
            ...
            if (
                ...
                and "numpy" in member.__name__  # it's a module
                and not member_name.startswith("_")  # inside NumPy
                and "numpy._core" not in member.__name__  # not private
                and  # outside _core
...
def _check___qualname__(obj) -> bool:
    ...
    return actual_obj is obj or (
        # for bound methods check qualname match
        module_name.startswith("numpy.random")
    ...
    while len(modules_queue) > 0:
        ...
        for member_name in dir(module):
            ...
            if (
                ...
                and "numpy._core" not in member.__name__  # skip modules
                and member not in visited_modules  # outside _core  # not visited yet
            ):
                modules_queue.append(member)
                visited_modules.add(member)
            elif (
                ...
                and not _check___qualname__(member)
                and member not in visited_functions
            ):
                incorrect_entries.append(
                    dict(
                        actual=member.__qualname__,
                        expected=member.__name__,
                    )
                )
                visited_functions.add(member)
