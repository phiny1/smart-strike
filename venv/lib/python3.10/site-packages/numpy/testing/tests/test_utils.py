        ...
        with pytest.raises(AssertionError):
            with np.errstate(all="raise"):
                np.testing.assert_array_equal(
                    np.array([1, 2, 3], np.float32), np.array([1, 1e-40, 3], np.float32)
                )
...
class TestEqual(TestArrayEqual):
    ...
    def test_object(self):
        ...
        a = np.array([datetime.datetime(2000, 1, 1), datetime.datetime(2000, 1, 2)])
...
class TestArrayAlmostEqual(_GenericTest):
    ...
    def test_subclass_2(self):
        ...
        all(z)
        b = np.array([1.0, 202]).view(MyArray)
        expected_msg = (
            "Mismatched elements: 1 / 2 (50%)\n"
...
class TestArrayAssertLess:
    ...
    def test_simple_items_and_array(self):
        x = np.array(
            [
                [621.345454, 390.5436, 43.54657, 626.4535],
                [54.54, 627.3399, 13.0, 405.5435],
        ...
        x = np.array(
            [
                [3.4536, 2390.5436, 435.54657, 324525.4535],
...
@pytest.mark.parametrize("assert_func", [assert_array_equal, assert_array_almost_equal])
def test_xy_rename(assert_func):
    ...
    with pytest.warns(DeprecationWarning, match=dep_message), pytest.raises(
        TypeError, match=type_message
    ):
        assert_func(1, x=1)
        assert_func(1, 2, y=2)
