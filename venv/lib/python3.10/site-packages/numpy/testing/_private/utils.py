...
import numpy as np
from numpy._core import intp, float32, empty, arange, array_repr, ndarray, isnat, array
...
def assert_array_compare(
    ...
    *,
    ...
    __tracebackhide__ = True  # Hide traceback for py.test
    from numpy._core import array2string, isnan, inf, errstate, all, max, object_
    ...
    try:
        ...
        if isnumber(x) and isnumber(y):
            ...
            if equal_nan and dt == y.dtype and hasattr(dt, "na_object"):
                is_nan = isinstance(dt.na_object, float) and np.isnan(dt.na_object)
                bool_errors = 0
                try:
                    bool(dt.na_object)
                except TypeError:
...
def nulp_diff(x, y, dtype=None):
    ...
    if not x.shape == y.shape:
        raise ValueError(
            "Arrays do not have the same shape: %s - %s" % (x.shape, y.shape)
        )
...
def assert_warns(warning_class, *args, **kwargs):
    ...
    **kwargs : Kwargs
    ...
    if not args and not kwargs:
        ...
        if "match" in kwargs:
            raise RuntimeError(
                "assert_warns does not use 'match' kwarg, " "use pytest.warns instead"
            )
