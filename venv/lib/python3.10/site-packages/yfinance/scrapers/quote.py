...
class FastInfo:
    ...
    def _get_1y_prices(self, fullDaysOnly=False):
        if self._prices_1y is None:
            ...
            try:
                ...
                self._today_open = pd.to_datetime(
                    ctp["regular"]["start"], unit="s", utc=True
                ).tz_convert(self.timezone)
                self._today_close = pd.to_datetime(
                    ctp["regular"]["end"], unit="s", utc=True
                ).tz_convert(self.timezone)
    ...
    def shares(self):
        ...
        shares = self._tkr.get_shares_full(
            start=pd.Timestamp.utcnow().date() - pd.Timedelta(days=548)
        )
        # if shares is None:
