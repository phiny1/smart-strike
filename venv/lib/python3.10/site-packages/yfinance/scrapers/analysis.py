...
class Analysis:
    ...
    def growth_estimates(self) -> pd.DataFrame:
        ...
        for item in self._earnings_trend:
            ...
            row = {
                "period": period,
                "stockTrend": item.get("growth", {}).get("raw", None),
            }
            data.append(row)
        ...
        for trend_name, trend_info in trends.items():
            if trend_info.get("estimates"):
                for estimate in trend_info["estimates"]:
                    ...
                    existing_row = next(
                        (row for row in data if row["period"] == period), None
                    )
                    if existing_row:
        ...
        self._growth_estimates = (
            pd.DataFrame(data).set_index("period").dropna(how="all")
        )
        return self._growth_estimates
    ...
    def _fetch_earnings_trend(self) -> None:
        try:
            ...
            self._earnings_trend = data["quoteSummary"]["result"][0]["earningsTrend"][
                "trend"
            ]
        except (TypeError, KeyError):
