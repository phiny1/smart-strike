...
class FundsData:
    ...
    def _parse_top_holdings(self, data) -> None:
        ...
        self._top_holdings = pd.DataFrame(
            {"Symbol": _symbol, "Name": _name, "Holding Percent": _holding_percent}
        ).set_index("Symbol")
        ...
        self._equity_holdings = pd.DataFrame(
            {
                ...
                self._symbol: [
                    self._parse_raw_values(
                        _equity_holdings.get("priceToEarnings", pd.NA)
                    ),
                    ...
                    self._parse_raw_values(
                        _equity_holdings.get("priceToCashflow", pd.NA)
                    ),
                    self._parse_raw_values(
                        _equity_holdings.get("medianMarketCap", pd.NA)
                    ),
                    self._parse_raw_values(
                ...
                "Category Average": [
                    self._parse_raw_values(
                        _equity_holdings.get("priceToEarningsCat", pd.NA)
                    ),
                    self._parse_raw_values(
                        _equity_holdings.get("priceToBookCat", pd.NA)
                    ),
                    self._parse_raw_values(
                        _equity_holdings.get("priceToSalesCat", pd.NA)
                    ),
                    self._parse_raw_values(
                        _equity_holdings.get("priceToCashflowCat", pd.NA)
                    ),
                    self._parse_raw_values(
                        _equity_holdings.get("medianMarketCapCat", pd.NA)
                    ),
                    self._parse_raw_values(
        ...
        self._bond_holdings = pd.DataFrame(
            {
                ...
                "Category Average": [
                    ...
                    self._parse_raw_values(
                        _bond_holdings.get("creditQualityCat", pd.NA)
                    ),
                ],
            }
        ).set_index("Average")
        ...
        self._fund_operations = pd.DataFrame(
            {
                ...
                self._symbol: [
                    self._parse_raw_values(
                        _fund_operations.get("annualReportExpenseRatio", pd.NA)
                    ),
                    self._parse_raw_values(
                        _fund_operations.get("annualHoldingsTurnover", pd.NA)
                    ),
                    self._parse_raw_values(
                        _fund_operations.get("totalNetAssets", pd.NA)
                    ),
                ],
                "Category Average": [
                    self._parse_raw_values(
                    ...
                    self._parse_raw_values(
                        _fund_operations_cat.get("totalNetAssets", pd.NA)
                    ),
                ],
            }
