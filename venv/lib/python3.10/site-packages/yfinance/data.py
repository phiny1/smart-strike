...
class YfData(metaclass=SingletonMeta):
    ...
    def _set_session(self, session):
        ...
        try:
            ...
            raise YFDataException(
                "request_cache sessions don't work with curl_cffi, which is necessary now for Yahoo API. Solution: stop setting session, let YF handle."
            )

        if not isinstance(session, requests.session.Session):
            raise YFDataException(
                f"Yahoo API requires curl_cffi session not {type(session)}. Solution: stop setting session, let YF handle."
            )
        ...
        try:
            if self._cookie_strategy == "csrf":
                utils.get_yf_logger().debug(
                    f"toggling cookie strategy {self._cookie_strategy} -> basic"
                )
                self._session.cookies.clear()
                self._cookie_strategy = "basic"
            else:
                utils.get_yf_logger().debug(
                    f"toggling cookie strategy {self._cookie_strategy} -> csrf"
                )
                self._cookie_strategy = "csrf"
            self._cookie = None
            self._crumb = None
    ...
    def _save_cookie_curlCffi(self):
        ...
        if len(yh_domains) > 1:
            utils.get_yf_logger().debug(
                f"Multiple Yahoo cookies, not sure which to cache: {yh_domains}"
            )
            return False
        if len(yh_domains) == 0:
            return False
    ...
    def get(self, url, params=None, timeout=30):
        return self._make_request(
            url, request_method=self._session.get, params=params, timeout=timeout
        )
