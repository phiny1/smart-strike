...
class TickerBase:
    ...
    def get_income_stmt(
        self, proxy=_SENTINEL_, as_dict=False, pretty=False, freq="yearly"
    ):
        """
        :Parameters:
            as_dict: bool
        ...
        if pretty:
            ...
            data.index = utils.camel2title(
                data.index, sep=" ", acronyms=["EBIT", "EBITDA", "EPS", "NI"]
            )
        if as_dict:
            return data.to_dict()
        return data
    ...
    def get_shares_full(self, start=None, end=None, proxy=_SENTINEL_):
        ...
        if start is not None:
            ...
            start = (
                pd.Timestamp.fromtimestamp(start_ts).tz_localize("UTC").tz_convert(tz)
            )
        if end is not None:
    ...
    def get_earnings_dates(self, limit=12, proxy=_SENTINEL_) -> Optional[pd.DataFrame]:
        ...
        df[columns_to_update] = (
            df[columns_to_update].astype("float64").replace(0.0, np.nan)
        )
        ...
        df.rename(
            columns={"Surprise (%)": "Surprise(%)"}, inplace=True
        )  # Compatibility
