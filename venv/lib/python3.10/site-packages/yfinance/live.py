...
class BaseWebSocket:
    def __init__(
        self, url: str = "wss://streamer.finance.yahoo.com/?version=2", verbose=True
    ):
        self.url = url
        ...
        while True:
            try:
                ...
                if self._subscriptions:
                    ...
                    if self.verbose:
                        print(
                            f"Heartbeat subscription sent for symbols: {self._subscriptions}"
                        )
            except Exception as e:
                self.logger.error(
                    "Error in heartbeat subscription: %s", e, exc_info=True
                )
                if self.verbose:
    ...
    async def listen(self, message_handler=None):
        ...
        while True:
            try:
                ...
                self.logger.error(
                    "Error while listening to messages: %s", e, exc_info=True
                )
                if self.verbose:
