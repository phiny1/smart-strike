...
class QueryBase(ABC):
    def __init__(
        ...
        operator: str,
        operand: Union[
            List["QueryBase"], Tuple[str, Tuple[Union[str, numbers.Real], ...]]
        ],
    ):
    ...
    def _validate_eq_operand(self, operand: List[Union[str, numbers.Real]]) -> None:
        ...
        if not any(
            operand[0] in fields_by_type
            for fields_by_type in self.valid_fields.values()
        ):
