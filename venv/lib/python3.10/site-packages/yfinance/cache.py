...
class _TzDBManager:
    ...
    def _initialise(cls, cache_dir=None):
        ...
        if not _os.path.isdir(cls._cache_dir):
            ...
            _os.access(cls._cache_dir, _os.R_OK)
            and _os.access(cls._cache_dir, _os.W_OK)
        ):
            raise _TzCacheException(
    ...
    def store(self, strategy, cookie):
        ...
        try:
            ...
            with db.atomic():
                ...
                _CookieSchema.insert(
                    strategy=strategy, cookie_bytes=cookie_pkl
                ).execute()
        except _peewee.IntegrityError:
            raise
...
class _ISINCache:
    ...
    def get_db(self):
        ...
        try:
            ...
            get_yf_logger().info(
                f"Failed to create ISINCache, reason: {err}. "
    ...
    def store(self, key, value):
        ...
        try:
            ...
            old_rows_query = _ISIN_KV.delete().where(
                (_ISIN_KV.value == value) & (_ISIN_KV.created_at < one_week_ago)
            )
            ...
            if old_value != value:
                ...
                with db.atomic():
                    q = _ISIN_KV.update(
                        value=value, created_at=_dt.datetime.now()
                    ).where(_ISIN_KV.key == key)
                    q.execute()
