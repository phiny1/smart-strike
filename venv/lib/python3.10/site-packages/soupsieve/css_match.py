...
class _DocumentNav:
    ...
    def assert_valid_input(cls, tag: Any) -> None:
        ...
        if not cls.is_tag(tag):
            raise TypeError(
                f"Expected a BeautifulSoup 'Tag', but instead received type {type(tag)}"
            )
    ...
    def is_processing_instruction(
        obj: bs4.element.PageElement | None,
    ...
    def is_iframe(self, el: bs4.Tag | None) -> bool:
        ...
        return bool(
            ((el.name if self.is_xml_tree(el) else util.lower(el.name)) == "iframe")
    ...
    def get_descendants(
        self, el: bs4.Tag | None, tags: bool = False, no_iframe: bool = False
    ) -> Iterator[bs4.element.PageElement]:
        """Get descendants."""
    ...
    def get_attribute_by_name(
        cls, el: bs4.Tag, name: str, default: str | Sequence[str] | None = None
    ...
    def get_text(self, el: bs4.Tag, no_iframe: bool = False) -> str:
        ...
        return "".join(
            [
                node
                for node in self.get_descendants(el, no_iframe=no_iframe)  # type: ignore[misc]
...
class CSSMatch(_DocumentNav):
    ...
    def find_bidi(self, el: bs4.Tag) -> int | None:
        ...
        for node in self.get_children(el):
            ...
            if self.is_tag(node):
                ...
                if (
                    (name and name in ("bdi", "script", "style", "textarea", "iframe"))
                    or not self.is_html_tag(node)
                    or direction is not None  # type: ignore[arg-type]
                ):
                    continue  # pragma: no cover
    ...
    def match_attribute_name(
        self, el: bs4.Tag, attr: str, prefix: str | None
    ) -> str | Sequence[str] | None:
    ...
    def match_future_child(
        self, parent: bs4.Tag, relation: ct.SelectorList, recursive: bool = False
    ) -> bool:
        """Match future child."""
    ...
    def match_lang(self, el: bs4.Tag, langs: tuple[ct.SelectorLang, ...]) -> bool:
        ...
        if found_lang is None and (
            not self.is_xml or (has_html_namespace and root and root.name == "html")
        ):
            # Find head
            found = False
            ...
            if found and parent is not None:
                for child2 in parent:
                    if (
                        isinstance(child2, bs4.Tag)
                        and self.get_tag(child2) == "meta"
                        and self.is_html_tag(parent)
                    ):
                        c_lang = False
                        content = None
                        for k, v in self.iter_attributes(child2):
                            ...
                            if c_lang and content:
                                found_lang = content
                                self.cached_meta_lang.append(
                                    (cast(str, root), cast(str, found_lang))
                                )
                                break
    ...
    def filter(self) -> list[bs4.Tag]:  # noqa A001
        ...
        return [
            tag
            for tag in self.get_contents(self.tag)
