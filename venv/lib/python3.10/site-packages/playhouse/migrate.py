...
class SchemaMigrator(object):
    ...
    def add_foreign_key_constraint(
        ...
        constraint = constraint_name or "fk_%s_%s_refs_%s" % (table, column_name, rel)
        ctx = (
            self.make_context()
            .literal("ALTER TABLE ")
            .sql(Entity(table))
...
class SqliteMigrator(SchemaMigrator):
    ...
    def _update_column(self, table, column_to_update, fn):
        columns = set(
            column.name.lower() for column in self.database.get_columns(table)
        )
